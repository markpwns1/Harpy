        ...
        public Evaluator(Parser interpreter)
        {
            ...
            var bracket = new Operation("l bracket")
            {
                ...
                unaryFunction = (none) => {
                    ...
                    {
                        return new Value(Value.BatchType.Int, "(" + outp.content + ")");
                    }

                    var temp = Variables.CreateTemporary(outp.content);
            ...
            {
                binaryFunction = (left, right) => new Value(Value.BatchType.String, left.content + right.content)
            };
                ...
                unaryFunction = (none) =>
                {
                    ...
                    else
                    {
                        if(p.CurrentToken == "l bracket")
                        {
                            ...
                            else
                            {
                                ...
                                string.Join(" ", args.Select(x => 
                                    (x.type == Value.BatchType.String && x.content.Contains(" ")) ? "\"" + x.content + "\"" : x.content)) + "\n";
                            }
                        ...
                        else if (p.CurrentToken == "add")
                        {
                            ...
                            var v = Evaluate();
                            var va = Variables.Get(name);
                            p.output += "set /a var_" + name + (va.depth > 0 ? Variables.suffix : "") + "+=" + v.content + "\n";
                            return Value.VOID;
                        }
                        ...
                        else if (p.CurrentToken == "string add")
                        {
                            ...
                            var v = Evaluate();
                            var va = Variables.Get(name);
                            p.output += "set var_" + name + (va.depth > 0 ? Variables.suffix : "") + "=!var_" + name + Variables.suffix + "!" + v.content + "\n";
                            return Value.VOID;
                        }
            ...
            var ifStatement = new Operation("if")
            {
                ...
                unaryFunction = (none) =>
                {
                    ...
                    string temp = Variables.CreateTemporary(condition.content);
                    ...
                    while(p.CurrentToken == "elseif")
                    {
                        ...
                        temp = Variables.CreateTemporary(condition.content);
            ...
            var whileLoop = new Operation("while")
            {
                ...
                unaryFunction = (none) =>
                {
                    ...
                    var condition = EvaluateCondition();
                    var temp = Variables.CreateTemporary(condition.content);
            ...
            var forLoop = new Operation("from")
            {
                ...
                unaryFunction = (none) =>
                {
                    ...
                    try
                    {
                        upwards = int.Parse(start.content) < int.Parse(end.content);
                    }
                    catch
                    {
                        upwards = true;
                    ...
                    if (usingVariable)
                    {
                        ...
                        reference = Variables.Create(varname, start, p.depth).content; // %var_x%
                        name = "var_" + varname; // var_x
                    }
                    else
                    {
                        reference = Variables.CreateTemporaryInt(start.content); // %temp_x%
                    ...
                    p.output += "if not " + reference + " " + (upwards ? "LSS" : "GTR") + " " + end.content + " goto loop" + loop + "_end\n";
                    
                    p.EvaluateBlock();

                    p.loopIDs.Pop();
                    
                    p.output += "set /a " + name + (usingVariable ? Variables.suffix : "") + "+=" + step.content + "\n";
            ...
            var and = new Operation("and")
            {
                binaryFunction = (left, right) =>
                {
                    ...
                    //int id = varid;
                    p.output += "if " + left.content + " ( if " + right.content + " ( \n";
            ...
            var or = new Operation("or")
            {
                binaryFunction = (left, right) =>
                {
                    ...
                    //int id = varid;
                    p.output += "if " + left.content + " set " + name + "=1==1\n";
                    p.output += "if " + right.content + " set " + name + "=1==1\n";
                    return new Value(Value.BatchType.Bool, reference);
                }
            };
            ...
            var returnStatement = new Operation("return")
            {
                ...
                unaryFunction = (none) =>
                {
                    ...
                    {
                        v = Evaluate();
                        p.output += "set " + (v.type == Value.BatchType.Int ? "/a " : "") + "\"return_value=" + v.content + "\"\n";
                    }
            ...
            var asStatement = new Operation("as")
            {
                ...
                unaryFunction = (left) =>
                {
                    ...
                    return new Value(Value.ParseType(p.GetIdentifier()), left.content);
                }
            };
