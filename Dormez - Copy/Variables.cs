    ...
    public static class Variables
    {
        ...
        public static string FlexibleTemporary(Value value)
        {
            if (value.type == Value.BatchType.Int)
                return CreateTemporaryInt(value.value);

            return CreateTemporary(value.value);
        }
        ...
        public static Value Create(string name, Value value)
        {
            ...
            parser.output += "set " + (value.type == Value.BatchType.Int ? "/a " : "") + "\"var_" + name + "=" + value.value + "\"\n";
        ...
        public static Value Create(string name, Value value, int depth)
        {
            ...
            parser.output += "set " + (value.type == Value.BatchType.Int ? "/a " : "") + "\"var_" + name + (depth > 0 ? suffix : "") + "=" + value.value + "\"\n";
            return GetReference(name);
        }
        ...
        public static Value Assign(string name, Value value)
        {
            ...
            parser.output += "set " + (value.type == Value.BatchType.Int ? "/a " : "") + "\"var_" + name + (found.depth > 0 ? suffix : "") + "=" + value.value + "\"\n";
            return GetReference(name);
        }
