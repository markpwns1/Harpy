...
namespace Harpy
{
    public static class StandardLibrary
    {
        ...
        {
            return new Value(Value.BatchType.String, Variables.CreateTemporaryInt(args[0].value));
        }
        ...
        public static Value BoolToString(Value[] args)
        {
            ...
            string temp = Variables.CreateTemporary(args[0].value);
        ...
        {
            string name = Variables.NameFromReference(args[0].value);
        ...
        public static Value CreateVariable(Value[] args)
        {
            ...
            Value.BatchType type = (Value.BatchType)Enum.Parse(typeof(Value.BatchType), Evaluator.ToTitleCase(args[1].value));
            Variables.Create(args[0].value, type, 0);
            return Value.VOID;
        }
        ...
        {
            Value.BatchType type = (Value.BatchType)Enum.Parse(typeof(Value.BatchType), Evaluator.ToTitleCase(args[1].value));
            Variables.Create(args[0].value, new Value(type, args[2].value), 0);
            return Value.VOID;
        }
        ...
        {
            return new Value(Value.BatchType.Indeterminate, args[0].value);
        }

        public static Value SetTitle(Value[] args)
        {
            p.EmitLn("set \"window_title=" + args[0].value + "\"");
            return new Value(Value.BatchType.Void, "title " + args[0].value);
        }
        ...
        {
            return new Value(Value.BatchType.Void, "echo | set /p ^=" + Lexer.Sanitize(args[0].value));
        }

        public static Value Print(Value[] args)
        {
            return new Value(Value.BatchType.Void, "echo:" + Lexer.Sanitize(args[0].value));
        }
        ...
        {
            return new Value(Value.ParseType(args[0].value), args[1].value);
        }

        public static Value GetVarType(Value[] args)
        {
            return new Value(Value.BatchType.String, Variables.Get(args[0].value).type.ToString().ToLower());
        }
        ...
        public static Value IsNull(Value[] args)
        {
            string tempBool = Variables.CreateTemporary("1==0");
            string name = Variables.NameFromTempReference(tempBool);
            p.EmitLn("if defined var_" + args[0].value + " set " + name + "=1==1");
            return new Value(Value.BatchType.Bool, tempBool);
        }

        public static Value VarExists(Value[] args)
        {
            return new Value(Value.BatchType.Bool, Variables.Exists(args[0].value) ? "1==1" : "1==0");
        }
        ...
        {
            p.EmitLn("echo ERROR: " + args[0].value);
            p.EmitLn("pause");
            p.EmitLn("exit");
            return Value.VOID;
        }
